...
namespace HJM.Chip8.MonoGameUI.Sound
{
    public class SoundInstance
    {
        ...
        private DynamicSoundEffectInstance dynamicSoundEffectInstance = new DynamicSoundEffectInstance(SAMPLE_RATE, (CHANNELS_COUNT == 2) ? AudioChannels.Stereo : AudioChannels.Mono);

        private const int SAMPLES_PER_BUFFER = 3000;
        private const int BYTES_PER_SAMPLE = 2;

        private float[,] workingBuffer = new float[CHANNELS_COUNT, SAMPLES_PER_BUFFER];
        private byte[] XNABuffer = new byte[CHANNELS_COUNT * SAMPLES_PER_BUFFER * BYTES_PER_SAMPLE];
